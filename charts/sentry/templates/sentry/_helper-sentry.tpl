...
sentry.conf.py: |-
  ...
  SENTRY_OPTIONS['filestore.profiles-options'] = {
      'access_key': os.getenv("PROFILES_S3_ACCESS_KEY_ID", {{ $profilesS3.access_key | default "" | quote }}),
      'secret_key': os.getenv("PROFILES_S3_SECRET_ACCESS_KEY", {{ $profilesS3.secret_key | default "" | quote }}),
      {{- if $profilesS3.bucket_name }}
      'bucket_name': {{ $profilesS3.bucket_name | quote }},
      {{- end }}
      {{- if $profilesS3.endpoint_url }}
      'endpoint_url': {{ $profilesS3.endpoint_url | quote }},
      {{- end }}
...
- name: install-nodestore-s3
  ...
  command:
    ...
    - -c
    - |
      pip install --target=/sentry-plugins https://github.com/stayallive/sentry-nodestore-s3/archive/main.zip
  volumeMounts:
    - name: sentry-plugins
...
{{- if .Values.nodestore.backend }}
- name: PYTHONPATH
  value: "/sentry-plugins:$PYTHONPATH"
{{- end }}
{{- end -}}
